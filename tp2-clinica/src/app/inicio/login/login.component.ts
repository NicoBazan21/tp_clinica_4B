          ...
          .then((user)=>
          {
            switch(user['rol'])
            {
              ...
              if(user.habilitado)
              {
                ...
                this.userService.sesionFirestore = new Especialista(user['id'], user['nombre'], user['apellido'], user['mail'], user['especialidad'], user['edad'], user['dni'], user['clave'], user['habilitado'], ' ', user['rol']);
                this.router.navigateByUrl('clinica');
              }
              else
              {
